...
export default function EditEvent() {
  ...
  useEffect(() => {
    Promise.all([fetchProductoras(), fetchContactos()]).then(() => {
      fetchEvento();
    });
  }, [id]);
  ...
  const fetchEvento = async () => {
    try {
      ...
      setSelectedProductora({ value: data.productora_id.toString(), label: data.productora_nombre });
      setSelectedContacto({ value: data.contacto_id.toString(), label: data.contacto_nombre });
      ...
      setFecha(data.fecha);
      setValorPersona(data.valor_por_persona.toString());
      setValorHoraExtra(data.valor_hora_extra?.toString() || '0');
      setCantidadPersonas(data.cantidad_personas.toString());
    } catch (e) {
  ...
  // Geocoding effect
  useEffect(() => {
    if (!direccion || direccion.length < 5 || initialLoading) return;
    ...
    <div className="min-h-screen pb-20">
      <header className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-10">
        <div className="flex items-center p-4 max-w-3xl mx-auto w-full">
          <button onClick={() => navigate('/agenda')} className="p-2 mr-2 rounded-full hover:bg-gray-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-[#DDDADA]/90 backdrop-blur-sm p-6 rounded-3xl shadow-lg border border-white/50">
          ...
          <div>
            ...
            <div className="relative mb-3">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/90 border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-white/90 border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
            />
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-4 py-3 rounded-xl bg-white/90 border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-lg shadow-md transition-colors disabled:opacity-70"
            >
